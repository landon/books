...
\mainmatter
\chapter{Graphs}
A \emph{graph}\marginnote{graph} is a collection of dots we call \emph{vertices}\marginnote{vertices} some of which are connected by curves we call \emph{edges}. \marginnote{edges}
...
If $G$ is a graph, then $V(G)$ is its set of vertices and $E(G)$ its set of edges. \marginnote{$V(G)$, $E(G)$}
We write $\card{G}$ for the number of vertices in $V(G)$ and $\size{G}$ for the number of edges in $E(G)$. \marginnote{$\card{G}$, $\size{G}$}
Two vertices
are \emph{adjacent} \marginnote{adjacent} if they are connected by an edge.  The set
of vertices to which $v$ is adjacent is its \emph{neighborhood}, written $N(v)$. \marginnote{neighborhood} \marginnote{$N(v)$}
For the size of $v$'s neighborhood $\card{N(v)}$, we write $d(v)$ and call this the \emph{degree} of $v$. \marginnote{$d(v)$}\marginnote{degree}
...
A graph $H$ is a \emph{subgraph} of a graph $G$, written $H \subseteq G$ if $V(H) \subseteq V(G)$ and $E(H) \subseteq E(G)$. \marginnote{subgraph, $\subseteq$}
When $H \subseteq G$, we say that $G$ \emph{contains} $H$. \marginnote{contains}  If $v \in V(G)$, then $G-v$ is the graph we get by removing $v$ and all edges incident to $v$ from $G$. \marginnote{$G-v$}
A graph is $k$-colorable if we can color its vertices with (at most) $k$ colors such that adjacent vertices receive different colors. \marginnote{$k$-colorable}
A $0$-colorable graph is \emph{empty}, a $1$-colorable graph is \emph{edgeless} and a $2$-colorable graph is \emph{bipartite}. \marginnote{empty}\marginnote{edgeless}\marginnote{bipartite}
...
\marginnote{chromatic number}\marginnote{$\chi(G)$}
...
any two vertices are adjacent. \marginnote{complete}
We can usually do much better by just arbitrarily coloring vertices, reusing colors when we can.  \marginnote{maximum degree}\marginnote{$\Delta(G)$}
...
A set of vertices $S$ in a graph $G$ is a \emph{clique} if the vertices in $S$ are pairwise adjacent.\marginnote{clique}
The \emph{clique number} of a graph $G$, written $\omega(G)$, is the number of vertices in a largest clique in $G$.\marginnote{$\omega(G)$}

\begin{theorem}\label{OmegaLowerBound}
If $G$ is a graph, then $\chi(G) \ge \omega(G)$.
\end{theorem}

A set of vertices $S$ in a graph $G$ is \emph{independent} if the vertices in $S$ are pairwise non-adjacent.\marginnote{independent}
The \emph{independence number} of a graph $G$, written $\alpha(G)$, is the number of vertices in a largest independent set in $G$.\marginnote{$\alpha(G)$}
