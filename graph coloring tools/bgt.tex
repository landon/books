...
The natural next questions is what happens when $f(V(G)) = 2\size{G}$, can $G$ be $f$-irreducible in this case?  The examples in Figure [insert me] show that the answer is yes.
We will understand the case of equality completely in \nameref{KernelMagicChapter} using a tool that improves Lemma \ref{BasicIrreducible} in terms of the ``maximum independent cover number''.
